...
/**
 * This command rotate the cursor clockwise by the degrees described by the parameter
 * (the angles are indicated as integers in the range [0, 360]);
 * @param <L> type of Location.
 */
public class RIGHT<L extends Location<L>> implements Command{
    private final Cursor<L> cursor;
    private final int angle;

    public RIGHT(Cursor<L> cursor, int angle) {
        this.cursor = cursor;
        this.angle = angle;
    }
    ...
    public String resultOfCommand() {
        return "The cursor turned right by "+angle+".";
    }
